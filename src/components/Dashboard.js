...
class Dashboard extends Component {
    ...
    componentDidMount() {
        let myPromise = fetch("/Data_example.json", {
            method: 'GET',
            headers: {
        ...
        myPromise.then(res => res.json()).then(json => {
            this.setState({ items: json.data, isLoaded: true });
        });
    ...
    render() {
        ...
        var ar = [];
        ar = items.map(item => item.place);
        var non_dup = new Set(ar);//removing duplicate 'place's using Set constructor
        non_dup = [...non_dup];
        //console.log(non_dup)
            ...
            return (
                <div >
                    ...
                    <div style={{ width: '100%', background: 'white', overflowX: 'hidden' }}>
                        ...
                        <div style={{ width: '100%', bottom: '0px', position: window.innerWidth > 768 ? 'fixed' : 'relative', background: 'black', textAlign: 'center', fontFamily: 'Helvetica', color: 'white', paddingTop: '15px', paddingBottom: '15px' }}>Data visualization using Plotly chart libary</div>
                    </div>
                </div >
            )
        }
